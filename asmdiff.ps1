...
if (-Not (Test-Path ".\pokeamethyst.gba"))
{
    Write-Host "File 'pokeamethyst.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokeamethyst.gba > .\pokeemerald.dump
