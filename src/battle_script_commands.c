...
static bool32 TryAegiFormChange(void)
{
    ...
    case SPECIES_AEGISLASH_BLADE: // Blade -> Shield
        ...
        break;
    case SPECIES_CLAYDOL: // Defense -> Attack
        if (IS_MOVE_STATUS(gCurrentMove))
            return FALSE;
        gBattleMons[gBattlerAttacker].species = SPECIES_CLAYDOL_ATTACK;
        break;
    case SPECIES_CLAYDOL_ATTACK: // Attack -> Defense
        if (gCurrentMove != MOVE_KINGS_SHIELD)
            return FALSE;
        gBattleMons[gBattlerAttacker].species = SPECIES_CLAYDOL;
        break;
    }
        ...
        if (gBattleControllerExecFlags == 0)
        {
             ...
             || GetMonData(&gPlayerParty[*expMonId], MON_DATA_SPECIES_OR_EGG) == SPECIES_EGG)
            {
                ...
                gBattleMoveDamage = 0; // used for exp
                  ...
                  || GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) >= GetLevelCap())
            {
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
